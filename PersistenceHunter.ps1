...
# Admin permission privilege check
if (([System.Security.Principal.WindowsIdentity]::GetCurrent().Groups -match 'S-1-5-32-544')) {
    Write-Host "`nRunning as admin`n" -ForegroundColor Yellow
}
else {
    Write-Host "`nRunning in unprivileged context`n" -ForegroundColor Yellow
}
    ...
    if ($mode -like "filter") {
        ...
        $tasksFiltered = $tasksFiltered | Where-Object {
            !(
                ...
                ($_.ExecuteSignature -eq "Valid" -and $_.ExecutePath -like "C:\ProgramData\Microsoft\Windows Defender\Platform*")
            )
        }
...
function Get-Startups{
    ...
    if ($mode -like "Filter") {
        ...
        foreach ($item in $startupObjects) {
            ...
            # Skip desktop.ini
            if ($item.FileName -ieq "desktop.ini") {
                continue
            }
...
if($mode -ne $null){
    if($mode -like "all"){
        ...
        # Print each object, skipping null or empty fields
        foreach ($obj in $outputReport) {
            Write-Host ""
    ...
    elseif($mode -like "verbose"){
        Write-Host "`nRunning in verbose mode...`n" -ForegroundColor Yellow

    }
    elseif($mode -like "registry"){
        Write-Host "`nRunning in registry mode...`n" -ForegroundColor Yellow

    }
    elseif($mode -like "tasks"){
        Write-Host "`nRunning in tasks mode...`n" -ForegroundColor Yellow

    }
    elseif($mode -like "services"){
        Write-Host "`nRunning in services mode...`n" -ForegroundColor Yellow

    }
    elseif($mode -like "auto"){
        Write-Host "`nRunning in auto mode...`n" -ForegroundColor Yellow

    }
    elseif($mode -like "csv"){
        Write-Host "`nRunning in csv mode...`n" -ForegroundColor Yellow
    }
    else{
        write-host "Error, unrecognized mode arguement. Exiting." -ForegroundColor Red
    }
}
else{
    Write-Host "Running in default configuration..." -ForegroundColor Yellow
}
